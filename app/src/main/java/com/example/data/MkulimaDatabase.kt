...
val MIGRATION_16_17 = object : Migration(16, 17) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `farm_settings` ADD COLUMN `automaticFeedDeductionEnabled` INTEGER NOT NULL DEFAULT 1")
...
val MIGRATION_19_20 = object : Migration(19, 20) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `farm_settings` ADD COLUMN `subscriptionTier` TEXT NOT NULL DEFAULT 'FREE'")
        db.execSQL("ALTER TABLE `farm_settings` ADD COLUMN `subscriptionStatus` TEXT NOT NULL DEFAULT 'ACTIVE'")
        db.execSQL("ALTER TABLE `farm_settings` ADD COLUMN `subscriptionExpiresAt` INTEGER NOT NULL DEFAULT 0")
    }
}
...
val MIGRATION_20_21 = object : Migration(20, 21) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("""
            CREATE TABLE IF NOT EXISTS `milk_usage_logs` (
                `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                `syncId` TEXT NOT NULL,
                `farmId` TEXT NOT NULL,
                `date` TEXT NOT NULL,
                `litresToCooperative` REAL NOT NULL,
                `litresHomeUse` REAL NOT NULL,
                `litresToCalves` REAL NOT NULL,
                `notes` TEXT,
                `updatedAt` INTEGER NOT NULL,
                `isDeleted` INTEGER NOT NULL
            )
...
val MIGRATION_22_23 = object : Migration(22, 23) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `worker_accounts` ADD COLUMN `canViewHome` INTEGER NOT NULL DEFAULT 1")
        db.execSQL("ALTER TABLE `worker_accounts` ADD COLUMN `canUseQuickActions` INTEGER NOT NULL DEFAULT 1")
        db.execSQL("ALTER TABLE `worker_accounts` ADD COLUMN `canCreateTasks` INTEGER NOT NULL DEFAULT 1")
        db.execSQL("ALTER TABLE `worker_accounts` ADD COLUMN `canSubmitRequests` INTEGER NOT NULL DEFAULT 1")
    }
}

val MIGRATION_23_24 = object : Migration(23, 24) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `farm_settings` ADD COLUMN `notificationsEnabled` INTEGER NOT NULL DEFAULT 1")
        db.execSQL("ALTER TABLE `farm_settings` ADD COLUMN `notifyMilkLogs` INTEGER NOT NULL DEFAULT 1")
        db.execSQL("ALTER TABLE `farm_settings` ADD COLUMN `notifyNewEntries` INTEGER NOT NULL DEFAULT 1")
        db.execSQL("ALTER TABLE `farm_settings` ADD COLUMN `notifyAccountChanges` INTEGER NOT NULL DEFAULT 1")
        db.execSQL("ALTER TABLE `farm_settings` ADD COLUMN `notifyDeletions` INTEGER NOT NULL DEFAULT 1")
        db.execSQL("ALTER TABLE `farm_settings` ADD COLUMN `notifyReminders` INTEGER NOT NULL DEFAULT 1")
    }
}

val MIGRATION_24_25 = object : Migration(24, 25) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `farm_tasks` ADD COLUMN `isRecurring` INTEGER NOT NULL DEFAULT 0")
        db.execSQL("ALTER TABLE `farm_tasks` ADD COLUMN `recurrenceInterval` TEXT NOT NULL DEFAULT ''")
        db.execSQL("ALTER TABLE `farm_tasks` ADD COLUMN `checklistJson` TEXT NOT NULL DEFAULT ''")
    }
}

val MIGRATION_25_26 = object : Migration(25, 26) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `finance_records` ADD COLUMN `targetUnit` TEXT NOT NULL DEFAULT 'General Farm'")
    }
}

val MIGRATION_26_27 = object : Migration(26, 27) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `farm_units` ADD COLUMN `isArchived` INTEGER NOT NULL DEFAULT 0")
    }
}
